...
public class JobInstanceRecoveryScheduler {
    ...
    private void recoverBatchInstance(JobInstance instance) {
         ...
         *
         * Because Yak Ops has restarted, JobMetricsMonitor.monitoringJobs is empty.
    ...
    private String buildRecoveryMessage(Long instanceId,
                                        ...
                                        ZetaJobStatusResolveResult result) {
        if (result == null) {
            return "Yak Ops 重启后执行任务状态恢复失败，Zeta 状态解析结果为空。"
        ...
        builder.append("Yak Ops 重启后执行任务状态恢复，发现该实例已不再处于 Zeta 运行中状态，已自动补偿本地状态。")
