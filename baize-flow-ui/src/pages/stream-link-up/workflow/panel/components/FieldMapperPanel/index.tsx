...
import "./index.less";
import FieldMapperInitGuide from "@/pages/batch-link-up/workflow/panel/components/FieldMapperPanel/FieldMapperInitGuide";
...
function FieldMapperPanel({
  ...
  const debouncedUpdateSinkFieldName = useDebouncedCallback(
    updateSinkFieldName,
    80
  );
  ...
  const handleSyncFields = useCallback(() => {
    ...
    if (!inputSchema.length) {
      message.warning("请先在上游节点选择表或填写 Query，并点击字段解析");
      return;
    }
      ...
      },
      {
        ...
        render: (_, record) => (
          <SinkFieldInput
            ...
            onChange={(newValue) =>
              debouncedUpdateSinkFieldName(record.key, newValue)
            }
          />
        ),
      },
    ],
    [SinkFieldInput, debouncedUpdateSinkFieldName]
  );
  ...
  return (
    ...
    >
      <section className="mb-2.5 rounded-[22px] border border-blue-100/80 bg-white p-3">
        {!hasInputSchema ? (
