package io.baize.flow.common.config;
...
public class ConfigValidator {
    ...
    void validateSingleChoice(Option option) {
        ...
        if (CollectionUtils.isEmpty(optionValues)) {
            throw new OptionValidationException(
                    ...
                    OptionUtil.getOptionKeys(Arrays.asList(singleChoiceOption)));
        }
        ...
        if (o != null && !optionValues.contains(o)) {
            throw new OptionValidationException(
                    ...
                    OptionUtil.getOptionKeys(Arrays.asList(singleChoiceOption)), o, optionValues);
        }
        ...
        if (value != null && !optionValues.contains(value)) {
            throw new OptionValidationException(
                    ...
                    OptionUtil.getOptionKeys(Arrays.asList(singleChoiceOption)), value, optionValues);
        }
    }
    ...
    void validate(RequiredOption.AbsolutelyRequiredOptions requiredOption) {
        ...
        throw new OptionValidationException(
                ...
                OptionUtil.getOptionKeys(absentOptions));
    }
    ...
    boolean validate(RequiredOption.BundledRequiredOptions bundledRequiredOptions) {
        ...
        throw new OptionValidationException(
                ...
                OptionUtil.getOptionKeys(bundledOptions), OptionUtil.getOptionKeys(present), OptionUtil.getOptionKeys(absent));
    }
    ...
    void validate(RequiredOption.ExclusiveRequiredOptions exclusiveRequiredOptions) {
        ...
        if (count == 0) {
            throw new OptionValidationException(
                    ...
                    OptionUtil.getOptionKeys(exclusiveRequiredOptions.getExclusiveOptions()));
        }
        if (count > 1) {
            throw new OptionValidationException(
                    "These options(%s) are mutually exclusive, allowing only one set(\"[] for a set\") of options to be configured.",
                    OptionUtil.getOptionKeys(presentOptions));
        }
    }
